...
public class CreateCheckpointActivity extends SherlockActivity {
    ...
    private LatLng currentInputGeoPoint;
    ...
    private void updateSexagesimal() {
        double lat = currentInputGeoPoint.latitude;
        double lng = currentInputGeoPoint.longitude;
    ...
    private void updateDecimal() {
        ...
        dLatDegreesFraction.setText(degreesFractionFormat.format((Math.abs(lat) % 1) * 1E6), BufferType.EDITABLE);
        dLngDegreesFraction.setText(degreesFractionFormat.format((Math.abs(lng) % 1) * 1E6), BufferType.EDITABLE);
    }
    ...
    private void onSaveCheckpoint() {
        try {
            checkpointManager.addCheckpoint(geoCache.getId(), name.getText().toString(), currentInputGeoPoint);
            finish();
        } catch (Exception e) {
    ...
    class SexagesimalListener extends TextChangeListener {
        ...
        public void afterTextChanged(Editable s) {
            try {
                ...
                currentInputGeoPoint = new LatLng(latitude, longitude);
                stopWatch();
                updateSexagesimalSeconds();
                updateDecimal();
                updateAzimuth();
